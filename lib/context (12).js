...
function buildContext(sock, message, extra = {}) {
    ...
    // ENHANCED: Channel-aware sender number extraction
    let senderNumber = cleanSender.replace('@s.whatsapp.net', '').replace('@lid', '');

    // Enhanced channel support - extract actual phone number from channel format
    if (isChannel && senderNumber.includes('@newsletter')) {
        // For channels, sender might be in format: "1203634025212339916@newsletter"
        // We need to extract just the phone number part
        const channelMatch = senderNumber.match(/(\d+)@newsletter/);
        if (channelMatch && channelMatch[1]) {
            const extractedNumber = channelMatch[1];
            
            // Try to match with owner number or LID
            if (settings.ownerNumber && extractedNumber.includes(settings.ownerNumber)) {
                senderNumber = settings.ownerNumber;
            } else if (global.ownerLid && extractedNumber.includes(global.ownerLid)) {
...
console.log(`🔍 Sudo Check - Sender: ${senderNumber}, CleanSender: ${cleanSender}, IsSudo: ${senderIsSudo}`);
